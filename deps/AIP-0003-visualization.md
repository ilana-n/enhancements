...
Users frequently run profiling experiments across multiple configurations to understand performance trade-offs. Currently, there is no built-in way to visualize AIPerf results, forcing users to manually export data and use external tools to create plots.
...
* Support static artifact generation (PNG)
...
2. Path to a directory containing multiple runs
3. Path to a single profile run results
4. Multiple paths to individual profile run results (for manual comparison, assuming they are located in different directories)
...
**Interactive HTML Mode (`--html`):**
- When `--html` flag is provided, **MUST** generate a self-contained interactive HTML report
- HTML file **MUST** embed all sweep data and enable client-side interactivity via JavaScript
- **MUST** support dynamic axis selection, run toggling, and plot type switching entirely in the browser
- HTML file **MUST** work offline without requiring a running server
- **MUST** be saved to `{sweep_dir}/visualizations/report.html`

All formats **MUST** work with both multi-run comparisons and single-run deep-dive analysis. The mode (multi-run vs single-run) **MUST** be auto-detected based on arguments or directory structure.

### REQ 3 Interactive Server Mode

AIPerf **MUST** provide an interactive web-based visualization server via `aiperf plot --host`. The server **MUST**:

**Core Functionality:**
- Launch a local web server (default port 8080) using Plotly Dash
- Support **dynamic axis selection**: Users choose any available metric for x-axis and y-axis with plots updating server-side
- Support **Log scale toggles** for x-axis and y-axis
- Support **run selection**: Toggle which runs to display via checkboxes
- Support **plot type switching**: Switch between scatter, line, bar, box plots with server-side regeneration
- Support **interactive controls**: Zoom, pan, hover for detailed values (via Plotly.js)
- Support **filtering**: Apply complex filters on metrics and parameters with server-side data processing
- Provide **export functionality**: Download current view as PNG or HTML

**Requirements:**
- **MUST** run locally without requiring external services or internet connectivity
- **MUST** use the same shared components as PNG and HTML modes where possible
- **MUST** respect user configuration for default plots from `~/.aiperf/config.yaml`
- **MUST** support both sweep comparison mode and single-run time-series analysis mode
- **SHOULD** provide URL output (e.g., `http://localhost:8080`) upon successful startup
- **MUST** allow graceful shutdown via Ctrl+C

The interactive server mode is intended for active exploratory analysis on local machines or remote environments where maximum flexibility is required.
...
```bash
# Default: Generate static PNG plots from ./artifacts
aiperf plot 
...
- Pareto curve (throughput vs latency trade-offs)
- Performance comparisons across swept parameters
...
visualization:
    ...
    multi_run_defaults:
      - pareto_curve
      - output_token_per_user_vs_concurrency
      - ttft_vs_prefill_throughput
      - output_token_throughput_vs_concurrency
    ...
    multi_run_plots:
      ...
      ttft_vs_prefill_throughput:
        ...
        description: "Time to first token vs prefill throughput"
        x: ttft_p50
      ...
      output_token_throughput_vs_concurrency:
        ...
        description: "Output token throughput scaling with concurrency"
        x: concurrency
...
## Visualization Configuration Export from HTML

**Description**: Allow users to export their interactive HTML session settings as an AIPerf configuration file.

**Workflow**:
1. User opens HTML report and explores visualizations interactively
